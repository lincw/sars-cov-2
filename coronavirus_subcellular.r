...
# bioid <- read.xlsx("~/Documents/INET-work/virus_network/references/PPIs/Laurent_BioID/media-1.xlsx", sheet = 1, startRow = 2)
...
total <- c(length(unique(has_sub$Ensembl)), length(unique(husci$Ensembl_ID)), length(unique(gordon_science$Ensembl_uniprotIDmapping)), length(unique(stukalov$ensemblID)), length(unique(li$ensemblID)), length(unique(nabeel$Prey_ensembl)), length(unique(bioid1$ensemblID)))
...
subset <- list(HPA = has_sub, # 19670
    ...
    BioID = unique(merge(bioid1[, c(4, 2)], has_sub, by.x = "ensemblID", by.y = "Ensembl", all.x = TRUE))
        )
write.xlsx(subset, file = file.path(path_final, "/toSummarize/subcellular_location/data/subcell_raw_count.xlsx"))
...
subset_m_melt$variable <- factor(subset_m_melt$variable, levels = c("HuSCI", "Gordon", "Stukalov", "Li", "Nabeel", "BioID", "HPA"))
...
f_pvalue_df <- data.frame(matrix(unlist(f_pvalue), ncol = 6, byrow = TRUE))
names(f_pvalue_df) <- names(subset_major_ct)[c(3:8)]
f_pvalue_adj <- p.adjust(unlist(f_pvalue), method = "fdr")
f_pvalue_adj_df <- data.frame(matrix(unlist(f_pvalue_adj), ncol = 6, byrow = TRUE))
names(f_pvalue_adj_df) <- names(subset_major_ct)[c(3:8)]

write.csv(cbind(subset_major_ct[c(1, 3:8, 2)], subset_major_pt[c(3:8, 2)], f_pvalue_df, f_pvalue_adj_df), file = file.path(path_final, "statistic_results/5interactome_subcell_major_location.csv"), row.names = FALSE)

anno <- data.frame(subcell = rep(subset_major_ct$subcell, each = 6),
    xstar = ifelse(unlist(f_pvalue) < 0.05, rep(1:6, 14), NA),
    ystar = ifelse(unlist(f_pvalue) < 0.05, as.vector(t(subset_major_pt[, c(3:8)])), NA),
    lab = ifelse(f_pvalue_adj < 0.05, "*", NA))

ggplot(subset_m_melt, aes(x = variable, y = value * 100)) +
    geom_bar(aes(fill = variable), stat = "identity") +
    scale_fill_manual(values=c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "brown", "grey")) +
...
ggsave(file.path(path_final, "/toSummarize/subcellular_location/figure/subcellular_major_location_withBioID.pdf"), width = 12, height = 10)
