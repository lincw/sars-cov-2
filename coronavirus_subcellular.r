...
source("~/Documents/INET-work/INETscripts/cov_4apms.r")
...
write.xlsx(subset, file = file.path(path_final, "/toSummarize/subcellular_location/data/raw_count.xlsx"))
...
subset_m_melt$variable <- factor(subset_m_melt$variable, levels = c("HuSCI", "Gordon", "Stukalov", "Li", "Nabeel", "HPA"))

anno <- read.csv(file = file.path(path_final, "/raw/subcell_p.csv"))
pdf(file = file.path(path_final, "Y2H_screening/20201104_final/figures/subcellular_major_location.pdf"), width = 12, height = 10)
ggplot(subset_m_melt, aes(x = variable, y = value * 100)) +
    geom_bar(aes(fill = variable), stat = "identity") +
    scale_fill_manual(values=c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "grey")) +
...
for (i in 1:14) {
    for (j in 3:7) {
        ...
        c_pvalue[[count]] <- chisq.test(dt_tmp)$p.value
        count <- count + 1
    }
}
f_pvalue_df <- data.frame(matrix(unlist(f_pvalue), ncol = 5, byrow = TRUE))
names(f_pvalue_df) <- names(subset_major_ct)[c(3:7)]
c_pvalue_df <- data.frame(matrix(unlist(c_pvalue), ncol = 5, byrow = TRUE))
names(c_pvalue_df) <- names(subset_major_ct)[c(3:7)]
write.csv(cbind(subset_major_ct[c(1, 3:7, 2)], subset_major_pt[c(3:7, 2)], f_pvalue_df, c_pvalue_df), file = file.path(path_final, "statistic_results/5interactome_subcell_major_location.csv"), row.names = FALSE)
