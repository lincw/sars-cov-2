...
## cRIC all
binary_rbp_cRIC <- table(binary_human %in% unique(rbp_cRIC$Gene.name))[2]
...
## cRIC
cat(paste0("Number of unique RNA-binding proteins in cRIC: ", length(unique(rbp_cRIC$Gene.name)) - 5), "\n") # SARS-CoV-2 proteins excluded
cat(paste0("Number of HuSCI identified proteins in cRIC: ", binary_rbp_cRIC), "\n")
cat(paste(c("HuSCI identified proteins in cRIC:", binary_human[binary_human %in% unique(rbp_cRIC$Gene.name)]), collapse = " "), "\n")
