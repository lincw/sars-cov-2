...
write.csv(bioid_rna, file = "~/Documents/INET-work/virus_network/statistic_results/bioid_tissue_rna.csv", row.names = F)
```
...
table_specificity <- rbind(HPA = table_all_s, HuSCI = table_husci_s, Gordon = table_gordon_s[c(1, 2, 5, 3, 4)], Stukalov = table_stukalov_s[c(1, 2, 5, 3, 4)], Li = table_li_s[c(1, 2, 5, 3, 4)], Nabeel = table_nabeel_s[c(1, 2, 5, 3, 4)], BioID = table_bioid_s[c(1, 2, 5, 3, 4)])
table_distribution <- rbind(HPA = table_all_d[table_d_order], HuSCI = table_husci_d[table_d_order], Gordon = table_gordon_d, Stukalov = table_stukalov_d[table_d_order], Li = table_li_d[table_d_order], Nabeel = table_nabeel_d, BioID = table_bioid_d[table_d_order])

pdf("~/documents/INET-work/virus_network/Y2H_screening/20201104_final/figures/tissue_specificity_withBioID.pdf", width = 10, height = 5)
par(mar = c(10, 6, 4, 2), mfrow = c(1, 2))
barplot(table_specificity[c(2:7, 1), c(2, 5, 4, 1)] * 100, beside = T, las = 2,
    ylab = "% of genes",
    main = "Specificity",
    ylim = c(0, 100), col = c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "brown", "grey"))
legend("topright", legend = c("HuSCI", "Gordon", "Stukalov", "Li", "Nabeel", "BioID", "HPA"),
    fill = c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "brown", "grey"), ncol = 1, cex = .75, bty = "n")
barplot(table_distribution[c(2:7, 1), c(1:4)] * 100, beside = T, las = 2,
    ylab = "% of genes",
    main = "Distribution",
    ylim = c(0, 100), col = c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "brown", "grey"))
legend("topright", legend = c("HuSCI", "Gordon", "Stukalov", "Li", "Nabeel", "BioID", "HPA"),
...
gp <- ggplot(df1_melt, aes(x = dataset, y = value)) +
    ...
    labs(x = "", y = "% of gene") +
    gp_style
ggsave(gp, file = "~/Documents/INET-work/virus_network/figure_results/5PPI_dorwie_percent.pdf", width = 5, height = 5)
# tissue specific gene in count
df2 <- as.data.frame(t(dorward_per[c(1:6, 9), ]))
...
ggsave(file = "~/Documents/INET-work/virus_network/figure_results/5PPI_dorwie_percent_individual_withBioID.pdf", width = 6, height = 6)
```
...
padj_value <- p.adjust(pvalue, method = "fdr")
pv <- matrix(padj_value, ncol = 9, byrow = T)
```